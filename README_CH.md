...
│   └── Tx_LowPower_echo/
├── bluetooth-receiver-host/       # 家电状态同步显示上位机程序
└── docs/                          # 说明文档和硬件电路图
    └── circuit-diagrams/
```
...
- 使用者需要在脚本开头根据本机环境设置 `$Root`、`$Out`、`$StageRoot`、`$Py`、`$ToWav`、`$PinnedJsonPath`、`$JsonSources` 和 `$DtdlDir`。
...
该目录用于存放家电状态同步显示上位机程序及通信说明文档。与 Supporting Information Fig. S36 所示链路一致，MATLAB App 不直接接收蓝牙空口数据，也不参与识别或控制决策；它只通过 PC 串口 / UART 接收蓝牙接收端输出的继电器状态、解析结果或状态字符串，并用于上位机界面显示。
...
| 家电状态同步显示上位机 | `bluetooth-receiver-host/ENVIRONMENT.md` | MATLAB App、UART 串口接收、家电状态同步显示 |

## 9. 官方程序下载入口

运行本文工作流程需要用到以下官方工具或软件包：
...
- 使用者应优先阅读根目录说明，再进入各功能模块的 `ENVIRONMENT.md` 查看具体环境配置。
- 各脚本中的数据输入目录、输出目录、Python 解释器路径和 ST 官方工具路径需要根据本机安装位置设置。
- IDE 编译输出、缓存文件、日志和临时文件不是运行工作流程的必要输入，可由本地环境重新生成。
