...
重要：`Tx_LowPower_echo` 不是一个完全脱离 STM32CubeU5 的独立工程。为了让 STM32CubeIDE 工程、ThreadX 中间件、HAL 驱动和相对路径都能正确解析，读者需要先下载 ST 官方 STM32CubeU5，然后将本目录放到官方包的以下位置：
...
6. 如果用于数据采集，设置为 `APP_RUN_MODE_DATA_CAPTURE`。
7. 如果用于识别部署，设置为 `APP_RUN_MODE_RECOGNITION`。
