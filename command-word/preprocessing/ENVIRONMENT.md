...
说明：实验中使用的原始脚本名为 `stdatalog_to_wav_batch_tree_PINNED_v3_COMPLETE_ACQFOLDER_WITH_PREP_FAILREPORT.ps1`。本仓库中为了便于阅读，将其整理为 `convert_stdatalog_to_wav.ps1`。如果复现者保留原始文件名，运行命令中的脚本名可替换为原始文件名。
...
其中 `<STDATALOG-PYSDK_ROOT>` 是复现者下载或克隆 ST 官方 STDATALOG-PYSDK 的目录。
...
```text
<STDATALOG-PYSDK_ROOT>\stdatalog-pysdk\stdatalog_to_wav_batch_tree_PINNED_v3_COMPLETE_ACQFOLDER_WITH_PREP_FAILREPORT.ps1
```
