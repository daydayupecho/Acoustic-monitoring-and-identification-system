...
本目录包含用于同步显示家电状态和蓝牙接收端解析结果的 MATLAB 上位机程序、通信说明文档和界面素材。MATLAB App 不直接接收蓝牙发送器发送的数据，而是通过 PC 串口接收蓝牙接收端输出的解析结果。
...
固件端命令词识别成功后，会通过蓝牙发送模块发送控制指令。蓝牙接收端接收并解析后，再通过 UART 输出给运行 MATLAB App 的 PC。
